    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (inputLine != "End")
            {
                ...
                {
                    Town town = new Town();
                    List<string> input = inputLine.Split(new char[]{'=', '>'}, StringSplitOptions.RemoveEmptyEntries).ToList();
            ...
            {
                IEnumerable<Student> studs = town.Students.OrderBy(x => x.RegistrationDate).ThenBy(z => z.Name)
                    .ThenBy(a => a.Email);
                while (studs.Any())
                {
